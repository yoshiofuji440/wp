...
add_filter( 'script_loader_tag', function( $ret ) {
	...
	*/
	...
	elseif( stripos( $ret, '/js/spotlight.bundle.js' ) !== false || stripos( $ret, '/js/floatbox/floatbox.js' ) !== false ) {
		$ret = str_replace( '><', ' defer><', $ret );
	}
		...
		if( stripos( $contents, 'class="wp-block-luxe-blocks-scroll-block' ) !== false ) {
			...
			if( isset( $luxe['amp'] ) ) {
				...
				if( stripos( $contents, 'dragscroll-on' ) !== false ) {
					preg_match_all( '/<div[^>]+?class="wp-block-luxe-blocks-scroll-block[^"]+?dragscroll-on[^"]*?"[^>]*?>\s*?<(figure|pre)[^>]+?>/im', $contents, $scroll_block_array );
					foreach( array_unique( $scroll_block_array[0] ) as $scroll_block ) {
