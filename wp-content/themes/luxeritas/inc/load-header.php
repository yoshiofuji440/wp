...
add_action( 'wp_head', function() {
	...
	if( !isset( $luxe['amp'] ) ) {
		$fonts_path = ( TURI !== SURI && $luxe['child_css_compress'] === 'bind' ) ? SURI : TURI;

		// async.min.css のプリロード
?>
<link rel="preload" as="style" type="text/css" href="<?php echo TURI; ?>/style.async.min.css" />
...
add_action( 'wp_head', function() use( $load_files ) {
	...
	if( isset( $luxe['awesome_load_async'] ) && $luxe['awesome_load_async'] === 'sync' && isset ( $luxe['awesome_load_css_file'] ) && $luxe['awesome_load_css_file'] === 'cdn' ) {
		global $awesome;
