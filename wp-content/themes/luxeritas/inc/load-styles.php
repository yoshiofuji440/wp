...
function thk_file_status_check() {
	...
	$load_files = array(
		...
		// PWA
		'serviceworker'	=> array( THK_HOME_PATH . 'luxe-serviceworker.js', THK_HOME_URL . 'luxe-serviceworker.js' ),
		'luxe-manifest'	=> array( THK_HOME_PATH . 'luxe-manifest.json', THK_HOME_URL . 'luxe-manifest.json' ),
	...
	if( $_is['ssl'] === true ) {
		if( isset( $luxe['pwa_enable'] ) ) {
			if( file_exists( $load_files['serviceworker'][0] ) === false ) {
				$status = true;
			}
		}

		if( isset( $luxe['pwa_manifest'] ) ) {
			if( file_exists( $load_files['luxe-manifest'][0] ) === false ) {
				$status = true;
			}
		}
	}

	if( $status === true && $_is['customize_preview'] === false ) {
		$fchk = true;
	...
	foreach( $files as $key => $value ) {
		if( file_exists( $value['check'] ) === true && file_exists( $value['compare'] ) === true ) {
			...
			if( $comptime > $chektime || $key === 'jcheck' || $key === 'jmcheck' ) {
				...
				if( $wp_filesystem->touch( $value['check'], $_SERVER['REQUEST_TIME'] ) === false || $wp_filesystem->touch( $value['compare'], $_SERVER['REQUEST_TIME'] ) === false) {
					if( defined( 'WP_DEBUG' ) === true && WP_DEBUG == true ) {
...
function thk_load_customize_preview() {
	...
	if( isset( $luxe['design_file'] ) && file_exists( SPATH . '/design/' . $luxe['design_file'] . '/style.css' ) === true ) {
		wp_enqueue_style( 'luxe-design', SDEL . '/design/' . $luxe['design_file'] . '/style.css', array(), false, 'all' );
 ...
 *---------------------------------------------------------------------------*/
	...
	if( !isset( $luxe['amp'] ) ) {
		...
		if( isset( $luxe['wp_block_library_load'] ) ) {
			...
			elseif( $luxe['wp_block_library_load'] === 'default' ) {
				wp_add_inline_style( 'wp-block-library-theme', thk_direct_style( TPATH . '/styles/luxe-blocks-style.min.css' ) );
			}
		}
		...
		if( $luxe['child_css_compress'] !== 'bind' || TPATH === SPATH ) {
			if( file_exists( $load_files['p-style-min'][0] ) === true && filesize( $load_files['p-style-min'][0] ) !== 0 ) {
				wp_enqueue_style( 'luxe', $load_files['p-style-min'][1], array(), false, 'all' );
				if( isset( $luxe['css_to_style'] ) ) {
...
if( !isset( $luxe['amp'] ) && !isset( $embed ) ) {
	...
	*/
	...
	if( $_is['customize_preview'] === false ) {
		if( $luxe['jquery_load'] !== 'luxeritas' ) {
			wp_enqueue_script( 'luxe', $load_files['p-luxe-min'][1], array(), false, false );
		}
	}
	...
	if( $luxe['jquery_load'] !== 'none' ) {
		...
		if(
			...
			if( $luxe['gallery_type'] === 'spotlight' ) {
				add_filter( 'the_content', 'add_spotlight', 11 );
				wp_enqueue_script( 'thk-spotlight', TDEL . '/js/spotlight.bundle.js', array(), false, false );
			}
			elseif( $luxe['gallery_type'] === 'strip' ) {
				add_filter( 'the_content', 'add_strip', 11 );
			}
			elseif( $luxe['gallery_type'] === 'tosrus' ) {
			...
			elseif( $luxe['gallery_type'] === 'floatbox' ) {
				wp_enqueue_script( 'thk-floatbox', TDEL . '/js/floatbox/floatbox.js', array(), false, false );
			}
		}
